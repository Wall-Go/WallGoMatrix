...
(* left-handed fermion *)
RepPsiL=CreateParticle[{1},"F",mf,"PsiL"];
RepPsiR=CreateParticle[{2},"F",mf,"PsiR"];
...
*)
ParticleList={RepPhi,RepChi,RepPsiL,RepPsiR,RepA};
LightParticleList={};
...
groupFactors={SUNN->2};
customParameters={ms2->mPhi^2,mf2->mPsi^2,g1->g,\[Lambda]->lam};
...
test["WallGo"][process]=generateWallGo[
	...
	{"PsiL"},
	{"PsiL"}
]/.{mv->0,mf->0}//Simplify//ApartSquareFree
...
((s1*test1-s2*test2)/.setMassesToZero//Simplify)/.(s1-s2)->0
...
(*
	output of matrix elements
*)
OutputFile="output/matrixElements.u1_higgs_yukawa.LL";
SetDirectory[NotebookDirectory[]];
MatrixElements=ExportMatrixElements[
	OutputFile,
	ParticleList,
...
{particles,parameters,MatrixElements}=ImportMatrixElements["output/matrixElements.u1_higgs_yukawa.LL.json"];
...
{(32 g^4 s^2)/(-mv+t)^2+(32 g^4 u^2)/(-mv+t)^2+(16 g^2 s^2 y^2)/((-ms+s) (-mv+t))+(16 g^2 u^2 y^2)/((-mv+t) (-ms+u))+(2 s^2 y^4)/(-ms+s)^2+(2 u^2 y^4)/(-ms+u)^2}//Length


Table[i*i,{i,(32 g^4 s^2)/(-mv+t)^2+(32 g^4 u^2)/(-mv+t)^2+(16 g^2 s^2 y^2)/((-ms+s) (-mv+t))+(16 g^2 u^2 y^2)/((-mv+t) (-ms+u))+(2 s^2 y^4)/(-ms+s)^2+(2 u^2 y^4)/(-ms+u)^2}]
